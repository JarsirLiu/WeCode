...
- 共享远端配置接口可能同时返回多个 provider 的产品数据；调用方必须带上目标模块，关闭的模块不能因另一个模块触发共享请求而获得产品结果。
- catalog 缓存必须按目标模块或 provider 隔离。一个启用模块的缓存不能绕过另一个模块的关闭状态。
- 复用 catalog、preview 或产品快照缓存前必须重新通过当前模块生命周期门禁；模块关闭后，旧缓存不能继续作为展示或购买依据。
- catalog 状态查询失败与明确的 `catalog-disabled` 必须分开记录；查询失败不得默认放行，也不得伪装成已确认关闭。购买入口在本轮 catalog 状态未完整确认时必须进入明确的加载或错误状态。
