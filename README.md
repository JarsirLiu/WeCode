...
<div align="center">
  <img src="public/logo/icons/1024x1024.png" alt="WeCode" width="128" height="128" />
</div>
<p align="center">
...
WeCode 是 AI 编程工作台，提供桌面应用、浏览器界面和终端 Agent。本仓库包含客户端、后端服务、共享 UI，以及 Agent CLI 与运行时源码。

## 更新

- 2026-9-23：更新至 WeCode v3.14.3 版本。
...
Agent 源码修改后，执行 `pnpm --filter @zcode/cli... build` 并重启服务。需要验证完整发行包时，按下方”WeCode 命令行版”打包章节解压运行。
...
安装：双击打开产物 DMG，将 WeCode 拖入"应用程序"。本地构建未签名，首次打开若被 macOS 拦截，执行：

```bash
sudo xattr -rd com.apple.quarantine /Applications/WeCode.app
```
